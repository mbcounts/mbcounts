...
SQL SECURITY DEFINER VIEW `vw_merit_badge_counselors` AS
select distinct `mbc`.`counselors_id` AS `counselors_id`,`mbc`.`meritbadges_id` AS `meritbadges_id`,`c`.`FirstName` AS `FirstName`,
left(`c`.`MiddleName`,1) AS `MiddleName`,`c`.`LastName` AS `LastName`,`c`.`Address1` AS `Address1`,`c`.`Address2` AS `Address2`,
`c`.`City` AS `City`,`c`.`State` AS `State`,`c`.`ZIPCode` AS `Zip`,`c`.`PhoneNo` AS `Phone`,`g`.`lat` AS `Lat`,`g`.`lon` AS `Lon`,
troop_only
from ((`counselors` `c`
join `meritbadgecounselors` `mbc` on ((`c`.`ID` = `mbc`.`counselors_id`)))
left join `geodatas` `g` on(( sha1( upper(trim(address1)) + ' ' + upper(trim(address2)) + ' ' + upper(trim(city)) + ' ' + upper(trim(state)) ) = g.sha1 )))
where ((now() <= `c`.`ExpireDate`) );


CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost`
SQL SECURITY DEFINER VIEW `vwcounselorslatlon` AS
select `v`.`counselors_id` AS `counselors_id`,`v`.`meritbadges_id` AS `meritbadges_id`,`v`.`FirstName` AS `FirstName`,
`v`.`MiddleName` AS `MiddleName`,`v`.`LastName` AS `LastName`,`v`.`Address1` AS `Address1`,`v`.`Address2` AS `Address2`,
`v`.`City` AS `City`,`v`.`State` AS `State`,`v`.`Zip` AS `Zip`,`v`.`Phone` AS `Phone`,`g`.`lat` AS `lat`,`g`.`lon` AS `lon`
from (`vw_merit_badge_counselors` `v` join `geodatas` `g`
